...
func TestDispatcherExtLocalSort(t *testing.T) {
	...
	nextStep, err := ext.GetNextStep(d, task)
	require.NoError(t, err)
	subtaskMetas, err := ext.OnNextSubtasksBatch(ctx, d, task, nextStep)
	require.NoError(t, err)
	require.Len(t, subtaskMetas, 1)
	task.Step, err = ext.GetNextStep(d, task)
	require.NoError(t, err)
...
func TestDispatcherExtGlobalSort(t *testing.T) {
	...
	nextStep, err := ext.GetNextStep(nil, task)
	require.NoError(t, err)

	subtaskMetas, err := ext.OnNextSubtasksBatch(ctx, d, task, nextStep)
	require.NoError(t, err)
	require.Len(t, subtaskMetas, 2)
	task.Step, err = ext.GetNextStep(nil, task)
	require.NoError(t, err)
